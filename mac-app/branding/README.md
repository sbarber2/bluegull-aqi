...
--window-size`. Generated by `gen-dmg-background.py` (needs Pillow: `pip
install pillow`) -- run it again after changing `DMG_WINDOW_W/H`,
`DMG_ICON_X/Y`, or `DMG_APPLINK_X/Y` in the root Makefile, since the arrow
and text positions are hand-tuned to those exact numbers. The script's
docstring has the details found by actually test-building a DMG and
screenshotting the real Finder window: the arrowhead needs an even gap to
each icon's bounding box (bluegull-aqi-2fb -- it originally overlapped
the Applications icon), and a Finder icon-view window's visible content
renders noticeably shorter than the requested `--window-size` height, so
text placed too close to the bottom edge gets clipped.
